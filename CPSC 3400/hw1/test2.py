#path to students.txt = /Users/shaunlee/PycharmProjects/untitled/students.txt
#read the file with student data
def processFile():
    with open('/Users/shaunlee/PycharmProjects/untitled/students.txt', 'r') as r:
              fileobj = r.readlines()    
    new_list = []
    for item in fileobj:
        ...
        for element in range(len(student_data)):
            if student_data[3] != '0':
                if student_data[element] in scale.keys():
                    total_gpa.append(scale[student_data[element]])
            else:
                total_gpa.append(0.0)
        results[student_id] = calcMean(total_gpa)
    return results
